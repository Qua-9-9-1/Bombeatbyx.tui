        ...
        for x in 0..width {
            ...
            match game_state.grid[idx] {
                ...
                Cell::Explosion { mut ticks_left } => {
                    ...
                    game_state.grid[idx] = if ticks_left == 0 {
                        Cell::Empty
                    } else {
...
fn explode_bomb(game_state: &mut GameState, bx: usize, by: usize, owner_id: u32) {
    ...
    game_state.grid[by * width + bx] = Cell::Explosion { ticks_left: 1 };
    ...
    for (dx, dy) in directions {
        for i in 1..=range {
            ...
            match game_state.grid[idx] {
                Cell::Wall => break,
                Cell::Brick => {
                    game_state.grid[idx] = Cell::Explosion { ticks_left: 1 };
                    break;
                }
...
pub fn check_deaths(game_state: &mut GameState) {
    for i in 0..game_state.players.len() {
        let (cx, cy, is_alive) = {
            ...
            (player.sub_x / 2, player.sub_y / 1, player.is_alive)
        };
