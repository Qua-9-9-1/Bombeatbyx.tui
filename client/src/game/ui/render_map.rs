...
pub fn draw_map(buffer: &mut Buffer, app: &App, rect: Rect) {
    ...
    for y in 0..app.game_state.height {
        for x in 0..app.game_state.width {
            ...
            draw_cell(buffer, cell, x, y, play_zone_x, play_zone_y, player_is_here && is_bomb);
        }
    }
...
fn draw_cell(buffer: &mut Buffer, cell: Cell, x: usize, y: usize, play_zone_x: u16, play_zone_y: u16, player_under_bomb: bool) {
    let bg_color = if player_under_bomb {
        ...
        match cell {
            ...
            Cell::Bomb { ticks_left, .. } => {
                if ticks_left <= 1 { Color::Red } else { Color::Indexed(234) }
            }
        }
    };
    ...
    buffer[(out_x, out_y)].set_symbol(" ").set_style(Style::default().bg(bg_color));
    buffer[(out_x + 1, out_y)].set_symbol(" ").set_style(Style::default().bg(bg_color));

    if let Cell::Bomb { .. } = cell {
        buffer.set_string(out_x, out_y, "💣", Style::default().bg(bg_color));
    } else {
        match cell {
            Cell::Wall => { buffer.set_string(out_x, out_y, "██", Style::default().bg(bg_color).fg(Color::Indexed(248))); }
            Cell::Brick => { buffer.set_string(out_x, out_y, "░░", Style::default().bg(bg_color).fg(Color::Rgb(205, 133, 63))); }
            Cell::Explosion { .. } => { buffer.set_string(out_x, out_y, "💥", Style::default().bg(bg_color)); }
            _ => {}
        }
    }
}

fn draw_player(buffer: &mut Buffer, player: &common::game::Player, play_zone_x: u16, play_zone_y: u16) {
