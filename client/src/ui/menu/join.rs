...
pub fn draw_join_room_menu(buffer: &mut Buffer, tui_area: Rect, app: &App) {
    ...
    let title = if ascii { " [ JOIN GAME ] " } else { " 🌐 JOIN GAME " };
    ...
    let mut lines = vec![
        Line::from(""),
        Line::from(Span::styled(format!("FILTER MODE (Q/D to toggle): < {} >", filter_str), Style::default().fg(Color::LightCyan).add_modifier(Modifier::BOLD))),
        Line::from(""),
    ];
    
    let header = format!("  {:<8} | {:<16} | {:<8} | {:<8} | {:<8}", "CODE", "HOST", "PLAYERS", "MODE", "TYPE");
    lines.push(Line::from(Span::styled(header, Style::default().fg(Color::Yellow).add_modifier(Modifier::UNDERLINED))));
    
    let mut filtered_rooms = Vec::new();
    if app.join_filter_mode == 0 || app.join_filter_mode == 1 {
        for r in &app.network.online_rooms {
            filtered_rooms.push((r.code.clone(), r.host_name.clone(), r.player_count, "Online".to_string(), if r.is_public { "Public" } else { "Private" }));
        }
    }
    ...
    if filtered_rooms.is_empty() {
        ...
        lines.push(Line::from(Span::styled("  No public rooms found.", Style::default().fg(Color::Red))));
        lines.push(Line::from("  Press [R] to refresh or [P] to join a room by code."));
        lines.push(Line::from(""));
    } else {
        for (idx, room) in filtered_rooms.iter().enumerate() {
            let row = format!("  {:<8} | {:<16} | {:<8} | {:<8} | {:<8}", room.0, room.1, format!("{}/8", room.2), room.3, room.4);
            if idx == app.join_cursor {
                lines.push(Line::from(Span::styled(format!("► {}", row), Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))));
            } else {
    ...
    let instruct = " [Enter] Join selected room   [P] Join room by code   [R] Refresh   [Esc] Back ";
